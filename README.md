...
- `td-shared` (file `<base>.vrp.json`): route-centered windows over the free-flow fastest matrix, audited for serveability under all 6 traffic overlays and minimally repaired (`metadata.tw_repair`). **These are exactly the windows of the base's 6 TDVRPTW subinstances.**
...
All windows guarantee individual serveability: a vehicle leaving the depot at the horizon start can serve any single customer within its window and return by the horizon end (at free flow for the static sets, under every traffic overlay for `td-shared`).
